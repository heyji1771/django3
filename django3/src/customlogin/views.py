...
def signup(request):
    if request.method == 'GET':
        ...
        # 유효한 값으로 채워져있는지 확인 (아이디 중복, 비밀번호 형식이 올바른지, 이메일 형식)
        if f.is_valid():
            ...
            if f.cleaned_data['password_check'] == f.cleaned_data['password']:
                #회원생성
                '''
                회원생성을 웨해 f.save()를 사용할 경우, 비밀번화가 암호화되지 않은 상태로 데이터베이스에 저장되기
                때문에 비밀번호 원본이 저장됨 따라서 회원생성에 사용할 수 있는 별도의 함수를 사용해야함
                
                ㅇ-> User : auth 어플리케이션에 구현된 모델 클래스
                ㅇ-> User.objects.create_user : 비밀번호를 암호화한 상태러 회원을 생성하는 함수
                '''
                
                # 관리자 계정이 만들어지게 됨 -> User.objects.create_superuser(username, email, password)
            ...
            else: #비밀번호 입력이 다른경우게 대한 처리
                return render(request, 'cl/signup.html', {'form':f , 'error' : '비밀번호가 다릅니다.'})
            
        else : # 유효하지 않은 입력값이 있을때의 처리
            return render(request, 'cl/signup.html', {'form':f, 'error':'형식이 맞지 않습니다.'})
    

#<로그인>
#GET - 로그인에 관한 입력을 할 수 있는 HTNL전달
...
def signin(request):
    if request.method == 'GET' :
        ...
        #회원조회 하는 함수를 사용해 아이디와 비밀번호가 동일한 회원이있는지 확인
        # is_valid를 사용해 유효성 검사를 할 경우, 아이디 중복에 걸려 항상 False가 뜸
        #따라서 오류가 발생했을때 입력을 전달하기 위한 폼객체 생성
        f = SigninForm(request.POST)
        #POST 변수에서 직접 데이터 꺼내기(아이디,비밀번호)
        id = request.POST.get('username')
        pw = request.POST.get('password')
        # 아이디랑 비밀번호가 일치하는 User 모델 클래스 객체 추출
        #authenticate(username, password) : 비밀번호를 암호화하고 아이디와 암호화된 비밀번화가 일치하는 User객체 추출
        #단, 아이디나 비밀번화가 틀린경우, None 값을 반환함
        ...
        if u:
            ...
            login(request, u)
            #nexturl : 이동할 URL주소를 저장하는 변수 (로그인이 필요로 하는 페이지에 로그인을 전달?)
...
#<로그아웃>
def signout(request):
    #logout(request) : 요청한 클라이언트이 User 저장 변수 값을 삭제함
    logout(request)
