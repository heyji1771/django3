...
#path(추가URL, 호출할 뷰)
#등록할 URL들은 앞단에 http://127.0.0.1:8000/ 주소가 붙음

urlpatterns = [
    #http://127.0.01:8000/a1 -> 관리자사이트 뷰 호출
    path('a1/', admin.site.urls),
    # http:// 127.0.01:8000/ -> index 뷰 호출
    path('', index, name = 'main'),
    #http://127.0.01:8000/list/ -> book_list 뷰 호출
    path('list/', book_list),
    #http://127.0.01:8000/숫자/ -> book_one 뷰 호출, book_idx변수에 숫자값이 들어감(꼭 마지막에 / 가 있어야함)
    path('<int:book_idx>/',book_one),
    #127.0.0.1:8000/vote 로 시작하는 URL들을 vote폴더의 urls.py에서 처리하도록 등록함
    path('vote/', include('vote.urls')),
    #127.0.0.1:8000/cl 로 시직하는 URL들을 customlogin폴더의 urls.py에서 처리하도록 등록함
    path('cl/',include('customlogin.urls')),
    # social_django 어플리케이션의 하위 URLConf 등록
    # 우리가 만든 어플리케이션이 아니므로 app_name변수에 어떤 값이 있는지 확인하기 힘듦
    # include함수의 namespace매개변수 : app_name변수를 무시하고 하위 그룹의 이름을 지정할 수 있음
    path('auth/', include('social_django.urls', namespace = 'social')),
    # #127.0.0.1:8000/blog 로 시직하는 URL들을 blog폴더의 urls.py에서 처리하도록 등록함
...
#웹클라이언트이 파일요청 URL 주소화 실제 파일경로를 연동함
