...
# 클래스 기반의 뷰를 만들때, 제네릭뷰를 상속받아서 장고에서 제공하는 뷰 기능을 사용
# 제네릭뷰 : 장고에서 제공하는 여러가지 상황별 뷰 기능을 구현한 클래스 

# <index : 글 목록이 뜨는 메인 페이지>
# ListView : 특정 모델클래스의 객체 목록을 다루는 기능이 구현된 뷰
class Index(ListView):
    #사용자에게 전달할 HTML 파일 경록
    template_name = 'blog/index.html'
    #리스트로 뽑을 모델클래스 지정
    model = Post
    #템플릿에 객체 리스트를 전댈할때 사용할 변수명
    context_obgect_name = 'list'
...
# DetailView : 특정 모델클래스 객체 한개를 추출해서 템플릿에 넘겨주는 제네릭뷰
# DetailView는 request외에 추가 매개변수를 사용해 추출할 객체를 찾음
# -> pk 변수 : id값 같은 고유값으로 객체를 찾을때 사용하는 변수
...
#LoginRequiredMixin : 사용자가 비로그인 상태로 이 뷰를 요청한 경우, 로그인페이지로 이동해주는 기능
...
class Posting(LoginRequiredMixin , FormView):
    ...
    # is_vaild()함수가 True를 반환한 뒤의 처리를 form_valid()에서 함
    ...
    def form_valid(self, form):
        ...
        # 데이터베이스에 바로 저장하지 않고 연동된 모델클래스로 변환
        p = form.save(commit = False)  #p : 사용자 입력으로 제목,카테고리,글내용이 채워진 post객체 (아직 데이터베이스에 저장되지 않음)
        # request.user : 요청한 클라이언트의 로그인정보 (User모델클래스 객체)
        #이 요청을 한 클라이언트의유저정보를 Post객체에 저장
        ...
        for  f in self.request.FILES.getlist('images'):
            # 새로운 PostImage 객체를 생성 (데이트베이스에 저장 X)
            pi = PostImage()
            # 이 요청으로 새로 만들어진 Post 객체와 연결
            pi.post = p
            # 사용자가 보낸 이미지파일 정보를 변수에 저장
            pi.image = f
            pi.save() # 데이터베이스에 새로운 PostImage객체 저장
                                               
        # 사용자가 넘겨준 첨부파일 마다PostFile 객체 생성
