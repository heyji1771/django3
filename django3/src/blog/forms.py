...
class PostForm(forms.ModelForm) :
    ...
    #이미지나 파일을 업로드하지않아도 글을 쓸수 있도록 커스텀 입력양식을 사용하는 것을 선택할 수 있도록 설정
    # -> required 매개변수를 False값을 설정
    
    # 하나의 파일업로드 입력양식에 여러개의 파일을 동시에 업로드할 수 있도록 위젯 설정
    # -> ClearableFileInput 위젯을 사용
    
    fiels = forms.FileField(required = False, widget = forms.ClearableFileInput(attrs = {'multiple' : True}))
