...
from vote.forms import ChoiceForm, QuestionForm
...
def index(request):
    ...
    #index.thml 파일에 Question객체리스트 전달 및 클라이언트에게 html파일 전송
    return render(request, 'vote/index.html' , {'objs' : objs})   


# <질문 선택 및 투표처리>
def detail(request, q_id):     #q_id : 사용자가 선택한 Question 객체의 id변수값
    # 웹 클라이언트의 요청방식에 따른 코드 처리
    #GET -> 설문조사 html  제공,
    #post -> 실제투표 반영
    #request.method : "GET" "POST" 중 웹클라이언트가 요청한 처리방식의 문자열이 저장됨
    if request.method == "GET" :
        # get_object_or_404(모델클래스, 조건)
        # 모델클래스에 조건을 검색해 1개의 객체 추출
        # 만약 객체가 없는경우, 클라이언트의 잘못된 접근으로 판단해 뷰함수를 종료하고 404번에러코드를 전달하는 함수
        
        #Question객체 중 id변수 값이 q_id와 같은 객체 추출 없으면 404번 에러코드 전달
        obj = get_object_or_404(Question, id=q_id)
        
        
        '''
        모델클래스A의 객체.모델클래스B_set : A모델클래스와 B모델클래스가 1:n관계(외래키연결)인 경우,
    해당 A객체와 연결된 B객체들을 대상으로 get(),all(),exclude(),filter() 함수들을 사용할수 있음
        Question 모델클래스와 Choice 모델클래스는 1:n관계이므로,
        Question 객체들은 자신과 연결된 Choice 객체들을 대상으로 get, all, exclude, filter를 사용할 수 있음
        '''
        # obj(Question객체)와 연결된 Choice객체를 모두 추출
        c_list = obj.choice_set.all() # 모델클래스이름은 모두 소문자로 
        print(c_list)
        return render(request, 'vote/detail.html' , {'q' : obj , 'c' : c_list})
    
    #POST 요청에 대한 처리 (투표 반영)
...
# 로그인이 되있는 클라이언트만 뷰함수를 쓸수있도록 설정
# 데코레이터 : URLConf를 통해 View함수가 호출될떄, 뷰가 실행되기 전에 먼저 실행되는 함수
# 데코레이터 적용 방식
# @데코레이터 함수 이름
# def 뷰함수(request):

#클래스 기반의 뷰는 데코레이터 대신에 XXXMixin 클래스를 상속받아서 사용

# login_required : 클라이언트가 비로그인상태일때, 로그인페이지로 이동하는 데코레이터 함수
...
def qregister(request):
    ...
    if request.method == "GET" :
        #Form 클래스 이용방법
        #객체 생성 후 as_p, as_table, as_list 함수를 호출하면 <input>태그를 자동생성해줌
        form = QuestionForm()
        print(form.as_p())
    ...
    #POST방식 -> 사용자 입력기반의 Question객체 생성
        ...
        if form.is_valid():
            '''
            폼객체.is_valid() : 사용자 입력이 유효한 경우 True반환, 추가적으로 cleaned_data 변수를 사용해 데이터를 추출할수 있음
            만약에 유효한값이 아닌경우 False 반환, 사용자의 입력을 다시받을 수 있도록 HTML 코드를 전달
            
            
            모델폼객체는 연동된 모델객체를 생성할 수 있음
            모델폼객체.save() : 반환값 생성된 모델객체. 내부적으로 데이터베이스에 새로운 객체가 저장됨
            
            현재 QuestionForm 객체로 title변수만 값이 작성되있어 데이터베이스에 바로 저장을 할 수 없음(pub_data변수값없음)          
            
            모델폼객체.save(commit = Flase) : 데이터베이스에 저장하지않고, 모델객체만 반환함
            ...
            # reverse 함수: 파이썬 코드내에서 별칭기반으로 URL을 찾을 수 있는 함수
...
def qupdate(request, q_id): 
    ...
    if request.method == "GET":
        #instance 매개변수 : 데이터베이스에 저장된 객체를 연동할 수 있는 변수, 연동한 모델폼객체는
        # HTML 코드로 변환시, 기본값이 객체 변수값으로 채워져있음
        form = QuestionForm(instance = q) # 값이 쒸워져 있는 상태로 제공함 (수정이니까)
        # 동일한 폼객체를 사용하기 떄문에 동일한 HTML파일을 사용함
        return render(request, 'vote/qform.html', {'form' : form.as_table()})
    
    #POST - 사용자가 입력한 데이터로 수정
    elif request.method == 'POST' :
        #q 객체의 데이터를 사용자의 입력으로 덮어쓰기
        form = QuestionForm(request.POST, instance = q) # 기존에 이런한 데이터가 있었는데.. 수정해라 
...
# Question 객체 삭제 (폼객체를 이용하지 않음)
@login_required
def qdelete(request,q_id):
    # 삭제 객체를 찾기
    q = get_object_or_404(Question, id=q_id)
    # 삭제 함수(delete) 호출
    print('객체 삭제전 id 변수: ' , q.id)
    q.delete()
    print('삭제후 id변수 : ', q.id)
    #return HttpResponseRedirect(reverse('vote:index')) # 삭제할 페이지를 보여주고 싶으면 render함수를 이용해서...  
    return render(request, 'vote/qdelete.html', {'qd' : q.title})  
    
    
    
    
# Choice 객체 추가
#GET - ChoiceForm 객체를 생성해 HTMl 파일에 전달
...
def cregister(request):
    if request.method == 'GET':
        ...
        # index 뷰로 이동
        #return HttpResponseRedirect(reverse('vote:index' ))
        # 삭제한 Choice객체가 연결된 Question 객체의 detail로 이동
...
# Choice 객체 수정
# 공통 : 수정할 대상을 추출
#GET - Choice 객체 기반으로 ChoiceForm객체 생성 후 HTML 전달
#POST - Choice 객체 + 입력데이터 기반으로 ChoiceForm 객체생성 바로 데이터베이스에 저장
# index 또는 detail로 이동
